...
import { getBasket } from '../../http/itemAPI';
...
export const Basket = observer(() => {
    ...
    useEffect(() => {
        getBasket().then(data => item.setBaskets(data))
    }, [])
    ...
    let prices = 0;
    {item.basket.map(price =>
        prices += Number(price.item.price)
    )}
    return (
        <Container
                    ...
                    <Row className="d-flex w-100">
                        <Col>
                            <div className="d-flex flex-row align-items-center">
                                <img src={import.meta.env.VITE_API_URL + product.item.img} width={50} />
                                <h1 className="pl-3">{product.item.name}</h1>
                            </div>
                        </Col>
                        <Col>
                            <div className="d-flex h-100 flex-row justify-content-end align-items-center">
                                <h2 className="font-weight-light">{product.item.price} рублей</h2>
                            </div>
                        </Col>
                    </Row>
